...
import { Globe, Download, X, Share2, FileJson } from 'lucide-react';

interface JSONExportOptionsModalProps {
    isOpen: boolean;
    onClose: () => void;
 ...
 * JSON导出选项Modal组件
 * 为移动端用户提供多种JSON导出方式：
 * 1. 在浏览器中打开 - 使用window.open()在新标签页打开JSON
 * 2. 保存到设备 - 使用HTML5 download属性触发下载
 * 3. 用其他应用打开 - 使用Web Share API分享
 */
export const JSONExportOptionsModal: React.FC<JSONExportOptionsModalProps> = ({
    isOpen,
    onClose,
    onOpenInBrowser,
    onSaveToDevice,
            ...
            <div className="relative w-full max-w-md bg-slate-800 rounded-2xl shadow-2xl border border-slate-700">
                ...
                <div className="flex items-center justify-between p-6 border-b border-slate-700">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-semibold text-white">导出 JSON 数据</h2>
                    </div>
                    <button
                        onClick={onClose}
                        disabled={isLoading}
                ...
                <div className="p-6 space-y-4">
                    ...
                    {isLoading && (
                        <div className="flex items-center justify-center gap-2 py-2">
                            ...
                            <span className="text-sm text-slate-400">正在生成JSON...</span>
                        </div>
                    )}
                </div>
